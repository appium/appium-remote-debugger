...
export default class WebKitRpcClient extends events.EventEmitter {
  ...
  receive (data) {
    ...
    const method = this.dataMethods[data.id] || data.method;
    if (!method) {
      return rejectCall(new WebKitRPCWarning('Did not find handler for the message'));
    }
    log.debug(`Found method '${method}' for message '${data.id}'`);
    switch (method) {
      ...
      default:
        ...
        if (!data.error && _.has(this.dataHandlers, msgId)) {
          return this.dataHandlers[data.id](data.result);
        }
        if (data.error && _.has(this.errorHandlers, msgId)) {
          return this.errorHandlers[data.id](data.error);
        }
        log.debug(`There is no promise scheduled for method '${method}' in message '${msgId}'`);
    }
  }
