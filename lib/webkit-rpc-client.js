...
export default class WebKitRpcClient extends events.EventEmitter {
  ...
  async send (command, opts = {}) {
    ...
    log.debug(`Sending WebKit data: ${truncateForPrinting(JSON.stringify(data), 200)}`);
    ...
    return new Promise(async (resolve, reject) => {
      ...
      this.dataHandlers[this.curMsgId.toString()] = resolve;
  ...
  receive (data) {
    log.debug(`Receiving WebKit data: ${truncateForPrinting(data)}`);
  ...
  setHandlers () {
    this.handlers = {
      'Runtime.evaluate': (data) => {
        ...
        // call the handler, and then get rid of it
      ...
      'Page.navigate': (data) => {
        let msgId = data.id;
        if (data.error) throw new Error(data.error);
        this.emit(RemoteDebugger._FRAME_NAVIGATED, true);
