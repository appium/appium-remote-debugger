...
export function getPossibleDebuggerAppKeys(bundleIds) {
  ...
  if (bundleIds.includes(WILDCARD_BUNDLE_ID)) {
    this.log.info('Returning all apps because the list of bundle identifiers includes a wildcard');
  ...
  this.log.debug(
    ...
    `You may also consider providing a wildcard ('*') in 'additionalWebviewBundleIds' to match ` +
    `all active apps.`
  );
  /** @type {string[]} */
  const proxiedAppIds = [];
