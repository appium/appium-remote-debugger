...
export async function executeAtomAsync (atom, args = [], frames = []) {
  ...
  const evaluate = async (method, opts) => await this.requireRpcClient().send(method, Object.assign({
...
export async function execute (command, override) {
  ...
  const res = await this.requireRpcClient().send('Runtime.evaluate', {
    expression: command,
    returnByValue: true,
    appIdKey: this.appIdKey,
    pageIdKey: this.pageIdKey,
...
export async function callFunction (objectId, fn, args) {
  ...
  const res = await this.requireRpcClient().send('Runtime.callFunctionOn', {
    objectId,
