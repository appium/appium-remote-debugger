...
export default class RpcClientSimulator extends RpcClient {
  ...
  async sendMessage (cmd, messageHandled = true) {
    let onSocketError;

    return await new B((resolve, reject) => {
      // most of the time we don't care when socket.write does
      // so give it an empty function
      const socketCb = messageHandled ? _.noop : resolve;
      ...
      try {
        ...
        const msg = `Could not create binary plist from data: ${e.message}`;
        log.error(msg);
      ...
      if (this.socket && this.connected) {
        ...
        const msg = 'Attempted to write data to socket after it was closed!';
        log.error(msg);
        return reject(new Error(msg));
      }
    })
  ...
  async receive (data) {
    ...
    while (dataLeftOver) {
      ...
      if (leftOver > 0) {
        ...
        const chunk = Buffer.alloc(leftOver);
