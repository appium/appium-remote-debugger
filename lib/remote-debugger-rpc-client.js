  ...
  async connect () {
    ...
    if (this.socketPath) {
      if (this.messageProxySocketPath) {
        ...
        log.debug(`Connecting to remote debugger through unix domain socket: '${this.messageProxySocketPath}'`);
      ...
      // tcp socket
      log.debug(`Connecting to remote debugger through TCP: ${this.host}:${this.port}`);
