  ...
  async connect () {
    ...
    if (this.socketPath) {
      if (this.messageProxySocketPath) {
        ...
        log.debug(`Connecting to remote debugger via proxy through unix domain socket: '${this.messageProxySocketPath}'`);
        this.socket = net.connect(this.messageProxySocketPath);
      ...
      // tcp socket
      log.debug(`Connecting to remote debugger ${this.messageProxyPort ? 'via proxy ' : ''}through TCP: ${this.host}:${this.port}`);
