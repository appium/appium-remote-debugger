...
import { truncateForPrinting } from './helpers';
...
export default class RemoteDebuggerRpcClient {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (this.messageHandler.hasSpecialMessageHandler(data.__selector)) {
        ...
        this.setSpecialMessageHandler(data.__selector, reject, function (...args) {
          log.debug(`Received response from socket send: ${JSON.stringify(args)}`);
          resolve(args);
        });
        ...
        this.setDataMessageHandler(this.curMsgId.toString(), reject, (value) => {
          log.debug(`Received data response from socket send: ${JSON.stringify(value)}`);
          resolve(value);
        });
      ...
      log.debug(truncateForPrinting(JSON.stringify(data)));
      ...
      try {
        ...
        let msg = `Could not create binary plist from data: ${e}`;
        log.error(msg);
        return reject(new Error(msg));
      }

      let onSocketError = (exception) => {
        if (this.connected) {
          log.error(`Socket error: ${exception.message}`);
        }
  ...
  receive (data) {
    log.debug('Receiving data from remote debugger');
    ...
    while (dataLeftOver) {
      ...
      for (let key of ['WIRMessageDataKey', 'WIRDestinationKey', 'WIRSocketDataKey']) {
        if (!_.isUndefined(plistCopy[key])) {
          plistCopy[key] = plistCopy[key].toString("utf8");
        }
      }

      if (plistCopy.__selector === "_rpc_applicationSentData:") {
        log.debug('Got applicationSentData response');
      } else {
        log.debug(JSON.stringify(plistCopy));
      }
