...
// Sometimes page initialization can take a long time, especiall in slow CI environments,
  ...
  async addTarget (err, app, targetInfo) {
    ...
    if (targetInfo.isProvisional) {
      ...
      if (!targetInfo.isPaused) {
        log.debug(`The target ${targetInfo.targetId}@${appIdKey} is not paused. This might be a problem`);
        return;
      }
      ...
      if (appTargetsMap) {
        try {
          await appTargetsMap.lock.acquire(targetInfo.targetId, async () => {
            try {
          ...
          log.warn(
            `Cannot complete the initialization of the paused target '${targetInfo.targetId}': ${e.message}`
          );
        }
      }
      return;
    ...
    try {
      ...
      log.warn(
        `Cannot complete the initialization of the created target '${targetInfo.targetId}': ${e.message}`
      );
    }
  }
