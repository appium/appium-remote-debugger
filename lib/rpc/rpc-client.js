...
export class RpcClient {
   ...
   */
  _getPendingPageTargetDetails(app, targetInfo) {
    const logInfo = (/** @type {string} */ message) => void log.info(
      `Skipping 'Target.targetCreated' event for app '${app}' ${message}: ${JSON.stringify(targetInfo)}`
    );
    if (!this._pendingTargetNotification) {
      return logInfo('with no pending request');
    }
    if (targetInfo.type !== 'page') {
      // TODO: We'll needs to handle 'frame' type for several domains.
