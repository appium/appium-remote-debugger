  ...
  async addTarget (err, app, targetInfo) {
    ...
    if (targetInfo.isProvisional) {
      ...
      try {
        await this.targets[appIdKey].lock.acquire(targetInfo.targetId, async () => {
          try {
            ...
            if (targetInfo.isPaused) {
              ...
              log.debug(
                `Provision target ${targetInfo.targetId}@${appIdKey} is not paused. This might be a problem`
              );
            }
          }
        });
        ...
        log.warn(
          `Cannot complete the initialization of the provisional target '${targetInfo.targetId}': ${e.message}`
        );
      }
      return;
    }
    ...
    try {
      ...
      log.warn(
        `Cannot complete the initialization of the new target '${targetInfo.targetId}': ${e.message}`
      );
    }
  }
