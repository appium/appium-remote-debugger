...
const PAGE_INIT_TIMEOUT_MS = 10000;
...
export class RpcClient {
  ...
  async addTarget (err, app, targetInfo) {
    ...
    const pageInitTimeoutMessage = `The page initialization timed out after ${pageInitTimeoutMs}ms. ` +
      `This could be caused by either a modal dialog obstructing the current page, ` +
      ...
      if (appTargetsMap) {
        await appTargetsMap.lock.acquire(appIdKey, async () => {
          try {
            ...
            ).timeout(pageInitTimeoutMs, pageInitTimeoutMessage);
          } catch (err) {
            log.error(err);
          }
        });
