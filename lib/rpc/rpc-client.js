...
export class RpcClient {
  ...
  async addTarget (err, app, targetInfo) {
    ...
    if ('frame' === targetInfo.type) {
      // TODO: We'll neede to handle this 'frame' type for several domains.
      ...
      log.debug(`Skip adding frame target type as a target. Received target info: ${JSON.stringify(targetInfo)}`);
      return;
    }
    log.debug(`Incoming target type is '${targetInfo.type}'`);
