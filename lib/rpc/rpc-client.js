...
export class RpcClient {
  ...
  async addTarget (err, app, targetInfo) {
    ...
    if (targetInfo.isProvisional) {
      ...
      try {
        await this.targets[appIdKey].lock.acquire(pageIdKey, async () => {
          try {
            if (await this._initializePage(appIdKey, pageIdKey, targetInfo.targetId)) {
              this._initializedPages.add(pageIdKey);
            }
          } finally {
    ...
    try {
      await this.targets[appIdKey].lock.acquire(pageIdKey, async () => {
        if (this._initializedPages.has(pageIdKey)) {
          log.debug(`Page '${pageIdKey}' is already initialized`);
          return;
        }

        try {
          if (await this._initializePage(appIdKey, pageIdKey)) {
            this._initializedPages.add(pageIdKey);
          }
        } finally {
          if (targetInfo.isPaused) {
  ...
  async _initializePage (appIdKey, pageIdKey, targetId) {
    ...
    for (const [domain, opts] of Object.entries(domainsToOptsMap)) {
      try {
        ...
        if (MISSING_TARGET_ERROR_PATTERN.test(err.message)) {
          return this.fullPageInitialization;
        }
      }
    }
    log.debug(
