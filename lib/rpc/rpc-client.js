...
export class RpcClient {
  ...
  constructor (opts = {}) {
    ...
    const maxOccupationTime = Math.max(this.pageLoadTimeoutMs ?? 0, PAGE_INIT_TIMEOUT_MS);
    this._pageSelectionLock = new AsyncLock({maxOccupationTime});
  ...
  async selectPage (appIdKey, pageIdKey, pageReadinessDetector) {
    await this._pageSelectionLock.acquire(`${appIdKey}:${pageIdKey}`, async () => {
      ...
      await this.waitForPage(appIdKey, pageIdKey, false);
    });
  }
   ...
   * @param {import('../types').AppIdKey} appIdKey
   * @param {import('../types').PageIdKey} pageIdKey
   * @param {boolean} [waitForPageSelection=true]
   * @returns {Promise<void>}
   */
  async waitForPage (appIdKey, pageIdKey, waitForPageSelection = true) {
    ...
    if (waitForPageSelection) {
      lockPromises.push(
        this._pageSelectionLock.acquire(`${appIdKey}:${pageIdKey}`, async () => await B.delay(0))
      );
    }
    ...
    if (durationMs > 10) {
      log.debug(
        `Waited ${durationMs}ms until the page ${pageIdKey}@${appIdKey} is ` +
        `${waitForPageSelection ? 'selected' : 'initialized'}`
      );
    }
  }
}
