...
const CONSOLE_LOGGING_CHANNELS = [
  'xml',
  'javascript',
  'network',
  'console-api',
...
export default class RpcClient {
  ...
  async initializePage (appIdKey, pageIdKey) {
    ...
    await this.send('getResourceTree', sendOpts, false);
    await this.send('enableNetwork', sendOpts, false);
    await this.send('Network.setResourceCachingDisabled', {
      ...sendOpts,
      disabled: false,
    }, false);
    await this.send('enableDomStorage', sendOpts, false);
    await this.send('enableDatabase', sendOpts, false);
    await this.send('enableIndexedDB', sendOpts, false);
    await this.send('enableCSS', sendOpts, false);
    await this.send('enableRuntime', sendOpts, false);
    await this.send('enableHeap', sendOpts, false);
    await this.send('enableMemory', sendOpts, false);
    await this.send('enableApplicationCache', sendOpts, false);
    ...
    await this.send('setBreakpointsActive', {
      ...sendOpts,
      active: true,
    }, false);
    await this.send('setPauseOnExceptions', {
      ...sendOpts,
      state: 'none',
    }, false);
    await this.send('setPauseOnAssertions', {
      ...sendOpts,
      enabled: false,
    }, false);
    await this.send('setAsyncStackTraceDepth', {
      ...sendOpts,
      depth: 200,
    }, false);
    await this.send('setPauseForInternalScripts', {
      ...sendOpts,
      shouldPause: false,
    }, false);

    await this.send('enableLayerTree', sendOpts, false);
    await this.send('enableWorker', sendOpts, false);
    await this.send('enableCanvas', sendOpts, false);
    await this.send('enableConsole', sendOpts, false);
    await this.send('DOM.getDocument', sendOpts, false);
    await this.send('getConsoleLoggingChannels', sendOpts, false);
    await this.send('inspectorInitialized', sendOpts, false);

    for (const source of CONSOLE_LOGGING_CHANNELS) {
      await this.send('setConsoleLoggingChannelLevel', {
        ...sendOpts,
        source,
        level: 'verbose',
      }, false);
