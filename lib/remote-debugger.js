    ...
    appLoop: for (let i = 0; i < maxTries; i++) {
      ...
      for (const attemptedAppIdKey of possibleAppIds) {
        try {
          ...
          // Sort the apps that the ones still has pageArray as a promise is at the bottom of the array
          const appsArray = _.values(this.appDict).sort((a, b) => {
            if (!_.isArray(a.pageArray) || !_.isArray(b.pageArray)) {
              return _.isArray(a.pageArray) ? -1 : 1;
            }
            return 0;
          });
          ...
          let found = false;
          dictLoop: for (const appDict of appsArray) {
