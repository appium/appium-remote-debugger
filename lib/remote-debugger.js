...
class RemoteDebugger extends events.EventEmitter {
  ...
  async selectApp (currentUrl = null, maxTries = SELECT_APP_RETRIES, ignoreAboutBlankUrl = false, selectOnlyFirstApp = false) {
    ...
    let appIdKey, pageDict, foundAtLeastAnApp = false, fullPageArray = [];
    appLoop: for (let i = 0; i < maxTries; i++) {
      if (foundAtLeastAnApp) {
        break;
      }
          ...
          dictLoop: for (let appDict of _.values(this.appDict)) {
            ...
            for (let dict of (appDict.pageDict || [])) {
              if ((!ignoreAboutBlankUrl || dict.url !== 'about:blank') && (!currentUrl || dict.url === currentUrl)) {
                ...
                foundAtLeastAnApp = true;
                pageDict = dict;
                appIdKey = appDict.id;
                if (this.appIdKey !== appDict.id) {
                  log.debug(`Received altered app id, updating from '${this.appIdKey}' to '${appIdKey}' with pages `);
                  this.appIdKey = appDict.id;
                }
                let pageArray = pageArrayFromDict(dict);
                log.debug(`selecting app ${this.appIdKey}: ${simpleStringify(pageArray)}`);
                let id = this.appIdKey.replace('PID:', '');
                let tmpPage = _.clone(dict);
                tmpPage.id = `${id}.${tmpPage.id}`;
                fullPageArray.push(tmpPage);
                break dictLoop;
              }
            }
          }

          // we have gotten the correct application by this point, so short circuit everything if we want an app
          if (selectOnlyFirstApp && foundAtLeastAnApp){
             break appLoop;
          }
        } catch (err) {
