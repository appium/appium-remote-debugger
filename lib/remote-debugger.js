...
import { appInfoFromDict, pageArrayFromDict, getDebuggerAppKey, checkParams,
         getScriptForAtom, truncateForPrinting, cancellableDelay } from './helpers';
...
class RemoteDebugger extends events.EventEmitter {
  ...
  async setConnectionKey () {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let connectCb = (apps) => {
        if (_.isUndefined(apps) || apps.length === 0) {
  ...
  async selectApp (maxTries = SELECT_APP_RETRIES) {
    ...
    pageDict = pageArrayFromDict(pageDict);
    log.debug(`Connected to app ${this.appIdKey}: ${JSON.stringify(pageDict)}`);
    return pageDict;
  }
  ...
  async executeAtom (atom, args, frames) {
    ...
    if (_.isUndefined(value)) {
      throw new Error(`Did not get OK result from execute(). Result was: ${JSON.stringify(value)}`);
    } else if (_.isString(value)) {
      return JSON.parse(value);
    } else if (_.isObject(value)) {
      return value;
    } else {
      throw new Error(`Result has unexpected type: (${typeof value}).`);
    }
  }
  ...
  async checkPageIsReady () {
    ...
    log.debug(`readyState was ${readyState}`);
  ...
  async waitForFrameNavigated () {
    return new Promise(async (resolve, reject) => {
      ...
      if (!this.useNewSafari || this.pageLoadMs >= 0) {
        ...
        this.navigationDelay = cancellableDelay(timeout);
        try {
  ...
  async startTimeline (fn) {
    ...
    return this.rpcClient.send('startTimeline', {
      appIdKey: this.appIdKey,
      pageIdKey: this.pageIdKey,
      debuggerType: this.debuggerType
    });
  ...
  async execute (command, override) {
    ...
    log.debug(`Sending javascript command ${truncateForPrinting(command)}`);
    return this.rpcClient.send('sendJSCommand', {
      command: command,
      appIdKey: this.appIdKey,
      pageIdKey: this.pageIdKey,
      debuggerType: this.debuggerType
  ...
  async callFunction (objId, fn, args) {
    ...
    return this.rpcClient.send('callJSFunction', {
      objId: objId,
      fn: fn,
      args: args,
      appIdKey: this.appIdKey,
