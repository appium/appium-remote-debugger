...
class RemoteDebugger extends events.EventEmitter {
  ...
  convertResult (res) {
    ...
    if (res.status && res.status !== 0) {
      ...
      throw new errors.JavaScriptError(`${res.value} (status: ${res.status})`);
    }

    // with either have an object with a `value` property,
    // or a plain object
    return util.hasValue(res.value) ? res.value : res;
  }
