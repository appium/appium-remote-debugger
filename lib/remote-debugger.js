...
class RemoteDebugger extends EventEmitter {
  ...
  async captureScreenshot() {
    ...
    const width = await this.executeAtom('execute_script', ['return window.innerWidth', []]);
    const height = await this.executeAtom('execute_script', ['return window.innerHeight', []]);

    return await this.rpcClient.send('Page.snapshotRect', {
      appIdKey: this.appIdKey,
      pageIdKey: this.pageIdKey,
      x: 0,
      y: 0,
